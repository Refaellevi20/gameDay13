...
function createCinema() {
    ...
    for (var i = 0; i < 7; i++) {
        cinema[i] = []
        for (var j = 0; j < 15; j++) {
            const cell = { isSeat: j !== 7 && j !==2 && j !==10  && i !==3 && i !==6 && j !==12 && j !==11 && j !==14 && j !==13}
            if (cell.isSeat) {
                cell.price = 5 + i
                cell.isBooked = false
            }
...
function showSeatDetails(pos) {
    ...
    const elHighlightBtn = elPopup.querySelector('.btn-highlight-seat')
...
//& the question 3 is like btn-book-seat looks the same algoritem  
function highlightAvailableSeatsAround(board, rowIdx, colIdx) {
    //* shorthand way to convert strings or other types to numbers
    rowIdx = +rowIdx
    colIdx = +colIdx
    ...
    for (var i = rowIdx - 1; i <= rowIdx + 1; i++) {
        ...
        for (var j = colIdx - 1; j <= colIdx + 1; j++) {
            ...
            //* if there is a seat and also if it is not booked
    ...
    gHighlightTimeout = setTimeout(() => {
        resetHighlight()
    }, 2500)
}

function resetHighlight() {
    var highlightedSeats = document.querySelectorAll('.highlight')
    highlightedSeats.forEach(elSeat => {
